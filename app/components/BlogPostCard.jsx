...
function BlogPostCard({image, title, category, description, to}) {
  return (
    <article className="relative flex duration-500 md:min-w-[14rem] bg-white rounded-md shadow-md md:flex-col lg:min-w-[17rem] lg:max-w-sm font-mont">
      ...
      <div className="absolute w-[90%] h-[90%] mx-auto left-0 right-0 my-auto top-0 bottom-0">
        <div 
          className="absolute shadow z-50 md:w-10 md:h-10 duration-500 bg-black bg-opacity-50 bg-cover rounded-md right-0 top-0 lg:hidden hover:w-[100%] hover:h-[100%]"
          style={{backgroundImage: "url('" + image + "')"}}>
        </div>
      ...
      <div className="z-40 px-4 py-4">
        ...
        <span className="block mb-3 font-medium lg:text-2xl">{title}</span>
        <p className="mb-4 leading-6 lg:leading-8 text-tiny lg:text-sm">{description}</p>
